...
static int CheckHwId(hx170dec_t * dev)
{
    ...
    while(numHw--)
    {
        ...
        {
            printk(KERN_INFO "hx170dec: Compatible HW found at offset 0x%08lx\n",
                   dev->dec_io_offset);
            return 1;
        }
    }
...
#endif
{
    ...
    irq_status_dec = readl(dev->dec_hwregs + X170_INTERRUPT_REGISTER_DEC);
    irq_status_pp = readl(dev->dec_hwregs + X170_INTERRUPT_REGISTER_PP);
