...
int wifi_set_power(struct wifi_platform_data *wifi_ctrl, int on, unsigned long msec)
{
	brcmf_dbg(TRACE, "Enter\n");
	if (wifi_ctrl && wifi_ctrl->set_power) {
		wifi_ctrl->set_power(on);
	}
	if (msec)
		mdelay(msec);
	return 0;
...
static int wifi_set_carddetect(struct wifi_platform_data *wifi_ctrl, int on)
{
	brcmf_dbg(TRACE, "Enter\n");
	if (wifi_ctrl && wifi_ctrl->set_carddetect) {
		wifi_ctrl->set_carddetect(on);
	}
	return 0;
}
...
//IAM
	...
	if (i == 0)
		return -ENXIO;
//IAM
	wifi_set_power(wifi_ctrl, 1, 0);	/* Power On */
	wifi_set_carddetect(wifi_ctrl, 1);	/* CardDetect (0->1) */
...
static int brcmf_sdio_pd_remove(struct platform_device *pdev)//IAM
{
	struct wifi_platform_data *wifi_ctrl =
		(struct wifi_platform_data *)(pdev->dev.platform_data);
	...
	wifi_set_power(wifi_ctrl, 0, 0);	/* Power Off */
	wifi_set_carddetect(wifi_ctrl, 0);	/* CardDetect (1->0) */
...
static struct platform_driver brcmf_sdio_pd = {
	...
	.remove		= brcmf_sdio_pd_remove,//IAM
	.driver		= {
//		.name	= "brcmf_sdio_pd"
		.name	= "bcmdhd_wlan"
	}
...
//IAM	sdio_unregister_driver(&brcmf_sdmmc_driver);
